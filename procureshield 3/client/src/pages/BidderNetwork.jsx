    ...
    <div>
      ...
      <ul className="mb-3 space-y-1 text-xs text-slate-600">
        {cluster.evidence.map((e) => (
          <li key={e}>• {WEIGHT_LABELS[e]?.label || e} (+{WEIGHT_LABELS[e]?.weight})</li>
        ))}
      </ul>

      <div className="mb-3 rounded-lg bg-slate-50 p-3">
        <h4 className="mb-1 text-xs font-semibold text-slate-700">AI Investigation Summary</h4>
        {investigating ? (
          <div className="flex items-center gap-2 text-xs text-slate-500">
            <Loader2 size={13} className="animate-spin" /> Generating summary...
          </div>
        ) : investigation ? (
