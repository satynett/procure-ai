...
import {
  ...
  ShieldQuestion, Loader2, ChevronRight, Share2,
} from "lucide-react";
...
export default function BidDetail() {
  ...
  useEffect(() => {
    ...
    api
      .aiAnalyze("relationship", {
        bidderA: data.bidder.company_name,
        bidderB: top.company_name,
        evidence: top.evidence,
        score: top.score,
        category: data.bid.risk_category,
      })
      .then(setAiSummary)
  ...
  const scoreBreakdown = relatedEdges[0]
    ? relatedEdges[0].evidence.map((e) => ({ label: WEIGHT_LABELS[e]?.label || e, value: WEIGHT_LABELS[e]?.weight || 0 }))
    : [];
  const scoreTotal = scoreBreakdown.reduce((s, x) => s + x.value, 0);
                ...
                {relatedEdges.map((e) => (
                  <div key={e.bidder_id} className="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-2">
                    <div>
                      ...
                      <div className="text-xs text-slate-400">{e.evidence.map((ev) => WEIGHT_LABELS[ev]?.label || ev).join(", ")}</div>
                    </div>
